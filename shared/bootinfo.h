/**
 * @file bootinfo.h
 * @brief ABI contract between UEFI Bootloader and Xenithra 64-bit OS Kernel.
 */
...
/* Magic signature for boot integrity verification: "XENITS64" */
#define XENITHRA_BOOT_MAGIC 0x34365354494E4558ULL
...
typedef struct {
    ...
    uint32_t pixel_format;        /* XenithraPixelFormat */
} XenithraFrameBuffer;
...
} XenithraMemoryDescriptor;

/**
 * @brief UEFI Memory Map information for kernel Physical Memory Manager (PMM)
 */
typedef struct {
    XenithraMemoryDescriptor* map;   /* Array of memory descriptors */
 ...
 */
typedef struct {
    uint64_t magic;                  /* Must equal XENITHRA_BOOT_MAGIC */
    uint32_t version;                /* Bootloader protocol version */
    uint32_t boot_mode;              /* 0 = Normal Desktop, 1 = Safe/Debug Mode, 2 = Diagnostics */
    
    XenithraFrameBuffer framebuffer; /* GOP FrameBuffer configuration */
    XenithraMemoryMap   memory_map;  /* Complete physical memory map */
